    ...
    const bool writeOutput)
{
    ...
    using array_t = float (&) [XDIM][YDIM][ZDIM*k];
    float *xmultipleRaw = new float [XDIM*YDIM*ZDIM*k];    
    array_t xmultiple = reinterpret_cast<array_t>(*xmultipleRaw);
    float *fmultipleRaw = new float [XDIM*YDIM*ZDIM*k];    
    array_t fmultiple = reinterpret_cast<array_t>(*fmultipleRaw);

for (int a = 0; a < XDIM; a++) {
    for (int b = 0; b < YDIM; b++) {
            for (int c = 0; c < ZDIM; c++) {
                for (int d = 0; d < k; d++){
                    xmultiple[a][b][d*ZDIM+c] = x[a][b][c];
                }
            }
        }
    }
    ...
    PARDISO (pt, &maxfct, &mnum, &mtype, &phase, &n,
        matrix.GetValues(), matrix.GetRowOffsets(), matrix.GetColumnIndices(),
        &idum, &nrhs, iparm, &msglvl, static_cast<void*>(&fmultiple[0][0][0]), &xmultiple[0][0][0], &error);
    if ( error != 0 )
